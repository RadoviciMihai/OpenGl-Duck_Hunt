...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_W)) logicSpace.y += 0.5 * deltaTime;
    if (window->KeyHold(GLFW_KEY_S)) logicSpace.y -= 0.5 * deltaTime;
    if (window->KeyHold(GLFW_KEY_D)) logicSpace.x += 0.5 * deltaTime;
    if (window->KeyHold(GLFW_KEY_A)) logicSpace.x -= 0.5 * deltaTime;
    if (window->KeyHold(GLFW_KEY_Z)) logicSpace.height -= 1.5f * deltaTime;
    if (window->KeyHold(GLFW_KEY_Z)) logicSpace.width -= 1.5f * deltaTime;
    if (window->KeyHold(GLFW_KEY_X)) logicSpace.height += 1.5f * deltaTime;
    if (window->KeyHold(GLFW_KEY_X)) logicSpace.width += 1.5f * deltaTime;
    logicSpace.x += lx;
    logicSpace.y += ly;
