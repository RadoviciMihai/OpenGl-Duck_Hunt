...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    GLint lpos = glGetUniformLocation(shader->program, "light_position");
    glUniform3fv(lpos, 1, glm::value_ptr(lightPosition));

    glm::vec3 eyePosition = GetSceneCamera()->m_transform->GetWorldPosition();
    // TODO(student): Set eye position (camera position) uniform

    GLint epos = glGetUniformLocation(shader->program, "eye_position");
    glUniform3fv(epos, 1, glm::value_ptr(eyePosition));
    ...
    glUniform1i(shpos, materialShininess);
    ...
    GLint cpos = glGetUniformLocation(shader->program, "object_color");
    glUniform3fv(cpos, 1, glm::value_ptr(color));
    // Bind model matrix
